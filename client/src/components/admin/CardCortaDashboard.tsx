...
const CardCortaDashboard = ({
  ...
  return (
    <div className="bg-neutral-200 rounded-xl p-2 relative">
      <Link to={`/${alias}/nuevo`} className="bg-green-500 rounded-xl absolute -top-2 right-2 w-10 h-10 flex justify-center items-center font-bold">
        {" "}
        <img className="w-8 h-8" src="/svg/plus.svg" alt="Agregar" />
      </Link>
     <Link to={`/${alias}`}> <img className="w-8 h-8" src={`/svg/${image}`} alt={titulo} />
